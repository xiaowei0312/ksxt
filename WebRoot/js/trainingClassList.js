...
//		CKEDITOR.instances.editor1.setData("test");
	...
	editModalSubmitBtnClicked : function() {
		...
		$.ajax({
			type : "POST",
			url : commodityListComp.basePath
					+ "/training/updateClassAjax.action",
			data : formData,
			async : false,
			cache : false,
			contentType : false,
	...
	editCommodityClicked : function(id,name,dirId,beginTime,endTime,status) {
		var modal = $('#editModal');
		...
		$('#commodityType_editModal').val(dirId);
	},
	delCommodityClicked : function(commodityId) {
		...
		$.ajax({
			type : "POST",
			url : commodityListComp.basePath
					+ "/training/deleteClassAjax.action",
			data : {
				id : commodityId,
			},
			success : function(e) {
