			...
			success : function(e){
				...
				if(a.logicCode == '-1'){
					...
					if(totalCommodityInThisType == 0){
						if(confirm("该方向下目前包含话题数量为：【"+totalCommodityInThisType+"】可以放心删除，确定删除该类型吗？")==false)
							return;
						typeListComp.delType(typeId);
					}else{
						alert("该方向下目前包含话题数量为：【" + totalCommodityInThisType+"】，无法删除!" );
					}
				}
			}
		});
