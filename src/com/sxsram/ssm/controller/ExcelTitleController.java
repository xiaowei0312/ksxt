...
import com.sxsram.ssm.service.ExcelTitleService;
...
public class ExcelTitleController {
	@Autowired
	private ExcelTitleService titleService;
	...
	@RequestMapping(value = "/importTitlesAjax", method = { RequestMethod.GET, RequestMethod.POST })
	@ResponseBody
	public String importStusAjax(HttpSession session, Model model, MultipartFile uploadFile) {
		...
		try {
			titleService.batchAddTitle(listob);
		} catch (Exception e) {
			e.printStackTrace();
			jsonResult.logicCode = "-1";
			jsonResult.resultMsg = e.getMessage();
